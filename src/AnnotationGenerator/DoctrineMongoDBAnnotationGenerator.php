    ...
    public function generateFieldAnnotations(string $className, string $fieldName): array
    {
        ...
        if (isset($type)) {
            ...
            if (CardinalitiesExtractor::CARDINALITY_0_1 === $field['cardinality']
                ...
                $annotations[] = sprintf('@MongoDB\ReferenceOne(targetDocument=%s::class, storeAs="id"))', $this->getRelationName($field['range']));
                ...
                $annotations[] = sprintf('@MongoDB\ReferenceMany(targetDocument=%s::class, storeAs="id")', $this->getRelationName($field['range']));
            }
        }
